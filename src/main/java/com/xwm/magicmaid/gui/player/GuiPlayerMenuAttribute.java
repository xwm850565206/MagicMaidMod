...
public class GuiPlayerMenuAttribute extends GuiScreen
{
    ...
    private List<List<GuiSkillWidget>> skillPages = new ArrayList<>();
    ...
    public void initGui()
    {
        ...
        if (player.hasCapability(CapabilityLoader.SKILL_CAPABILITY, null)) {
            ...
            if (skillCapability != null) {
                int total = skillCapability.getAttributeSkills().size();
                int k = 0; // 一页4个
                List<GuiSkillWidget> skillRects = new ArrayList<>();
                for (ISkill skill : skillCapability.getAttributeSkills()) {
                    int t = k % 4;
                    int x = offsetLeft + (t % 2) * (65 + gapx);
                    int y = offsetTop + (t/2) * (60 + gapy);
                    skillRects.add(new GuiSkillWidget(i + 4, i + x, j + y, skill));
                    k++;
                    if (k % 4 == 0) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        // 画技能
        List<GuiSkillWidget> page = skillPages.get(currPage);
        for (GuiSkillWidget skillRect : page) {
